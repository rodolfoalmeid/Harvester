...
GARAGE_VER="https://garagehq.deuxfleurs.fr/_releases/v2.1.0/x86_64-unknown-linux-musl/garage"
SRV_IP="192.168.68.28"
S3_REGION="homelab"
BUCKET_NAME="harvbucket"
...
echo "--- Starting S3 Bucket Server Configuration ---"
...
echo "[3/7] Installing Garage S3 Bucket..."
wget -q -O garage $GARAGE_VER
if [ $? -ne 0 ]; then
    echo "❌ Error: Fail to download Garage Binary. Check the URL the network connectivity."
    exit 1
fi
chmod +x garage
...
if systemctl is-active --quiet "$S3_SERVICE"; then
    echo "      $S3_SERVICE is running."
else
    echo "      $S3_SERVICE is NOT running. Check logs with: journalctl -u $S3_SERVICE"
    exit 1
fi
sleep 5

# 7. Initialize Layout
echo "[7/7] Initializing Garage Layout..."
